...
export default function WalletConnect({ onConnect }: Props) {
  ...
  const handleWalletConnect = async () => {
    ...
    try {
      ...
      // Step 3 — verify signature client-side using ethers
  ...
  const handleKeyImport = async () => {
    if (!keyValue || !password) {
      setError("Both private key and password are required.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    <div className="card w-full max-w-md p-6">
      ...
      <div className="flex gap-px bg-[#D8D4CC] rounded-md overflow-hidden mb-6">
        {(["wallet", "key"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => { setTab(t); setError(""); setStep("connect"); }}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className={`flex items-center gap-1.5 font-mono text-xs ${
              step === "connect" ? "text-navy" : "text-green"
            }`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                step === "connect" ? "bg-navy text-white" : "bg-green text-white"
              }`}>
                {step === "connect" ? "1" : "✓"}
              </span>
              Connect
            ...
            <div className={`flex items-center gap-1.5 font-mono text-xs ${
              step === "sign" ? "text-navy" : "text-[#6B6860]"
            }`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                step === "sign" ? "bg-navy text-white" : "bg-[#D8D4CC] text-[#6B6860]"
              }`}>
                2
              </span>
              Sign
          ...
          <p className="font-mono text-xs text-[#6B6860] leading-relaxed">
            {step === "connect"
              ...
              : "Check your wallet — a signing request has been sent. Sign the message to verify ownership and continue."}
          </p>
        ...
        <div className="flex flex-col gap-4">
          <div className="p-3 rounded bg-crimson/8 border border-crimson/20">
            <p className="font-mono text-xs text-crimson leading-relaxed">
              ⚠️ Your key is encrypted in-browser with AES-256 before any storage.
              We never store or transmit your plaintext key. Use a strong password.
            </p>
          </div>
          <div>
            ...
            <input
              ...
              placeholder="0x..."
              className="w-full font-mono text-xs bg-background border border-[#D8D4CC] rounded px-3 py-2.5
      ...
      {error && (
        <p className="mt-3 font-mono text-xs text-crimson bg-crimson/8 border border-crimson/20 rounded px-3 py-2">
          {error}
        </p>
      )}
    </div>
